...
However, we can use select with a default clause to implement non-blocking
...
func main() {
	...
	// If a value is available on messages then select will take the <-messages
	// case with that value. If not it will immediately take the default case.
	select {
	case msg := <-messages:
	...
	// Here msg cannot be sent to the messages channel, because the channel
